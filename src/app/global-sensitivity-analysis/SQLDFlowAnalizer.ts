...
let errorInModel: boolean = false;
let analyseInProgress: boolean = true;
let enabledNodes: any = [];
let alreadyProcessed: Array<string> = [];
let overlaysMap: { [id: string]: any } = {};

export class GlobalSensitivityAnalyser {
  ...
  public static analizeSQLDFlow(element: any, registry: any, canvas: any, overlays: any, eventBus: any, http: HttpClient) {
    ...
    const info = dataFlowAnalysis(element);
    ...
    if (errors.length) {
      GlobalSensitivityAnalyser.analysisError.emit(errors);
      $('.gs-analysis-spinner').hide();
      return;
    }

    enabledNodes = processingNodes.filter((nodeId: string) => alreadyProcessed.indexOf(nodeId) < 0 && invDataFlowEdges[nodeId].every((predId: string) => dataDefStatements[predId]));

    for (let nodeId of enabledNodes) {
      GlobalSensitivityAnalyser.analyzeProcessingNode(nodeId, eventBus, dataDefStatements, outputDefStatements, dataFlowEdges, invDataFlowEdges, registry, canvas, overlays, overlaysMap, http);
    }

  }

  static processTasksForAnalysis(element, canvas, overlays, dataDefStatements, dataFlowEdges, outputDefStatements, invDataFlowEdges, eventBus, registry, http) {

    if (is(element.businessObject, 'bpmn:Task') && !analyseInProgress) {
      ...
      if (enabledNodes.indexOf(node.id) >= 0 && !errorInModel) {
        ...
        if (dataFlowEdges[outputDataId] && dataFlowEdges[outputDataId].length > 0) {
          ...
          for (let nodeId of newlyEnabledNodes) {
            GlobalSensitivityAnalyser.analyzeProcessingNode(nodeId, eventBus, dataDefStatements, outputDefStatements, dataFlowEdges, invDataFlowEdges, registry, canvas, overlays, overlaysMap, http);
          }
        ...
        if (enabledNodes.length === 0) {
          ...
          if (!$.isEmptyObject(dc)) {
            ...
            $('#messageModal').find('.modal-title').text('Global sensitivity analysis');
            $('#messageModal').find('.modal-body').html(resultTable);
            $('#messageModal').modal('show');

            GlobalSensitivityAnalyser.analysisCompleted.emit({ node: { id: 'result', name: 'Result Table' }, overlayHtml: $(resultTable) });
  ...
  static analyzeProcessingNode(nodeId: string, eventBus: any, dataDefStatements: { [id: string]: string }, outputDefStatements: { [id: string]: string }, dataFlowEdges: any, invDataFlowEdges: any, registry: any, canvas: any, overlays: any, overlaysMap: any, http: HttpClient) {

    const node = registry.get(nodeId).businessObject;

    if (!node.sqlScript) {
      GlobalSensitivityAnalyser.analysisError.emit([{ error: 'SQL script not found', id: nodeId }]);
      $('.gs-analysis-spinner').hide();
      return;
    }
    ...
    if (result.parse_tree.length) {
      ...
      if (result.parse_tree[0].CreateFunctionStmt) {
        ...
        if (embeddedQuery.parse_tree.length) {
          ...
          http.post(config.backend.host + '/rest/sensitivities/global-sensitivity-analysis', { schema: obj_schema, query: obj_query }, AuthService.loadRequestOptions({ observe: 'response' })).subscribe(
            (success: HttpResponse<any>) => {
              if (success.status === 200) {
                ...
                GlobalSensitivityAnalyser.analysisCompleted.emit({ node: node, overlayHtml: overlayHtml });

                const element = registry.get(nodeId);
                GlobalSensitivityAnalyser.processTasksForAnalysis(element, canvas, overlays, dataDefStatements, dataFlowEdges, outputDefStatements, invDataFlowEdges, eventBus, registry, http);
              }
            },
            fail => {
              $('.gs-analysis-spinner').hide();
          ...
          $('.gs-analysis-spinner').hide();
          GlobalSensitivityAnalyser.analysisError.emit([{ error: `${embeddedQuery.error.message}`, id: nodeId }]);
          return;
        }

      } else {
        $('.gs-analysis-spinner').hide();
        GlobalSensitivityAnalyser.analysisError.emit([{ error: `Stored procedure not found`, id: nodeId }]);
        return;
      }

    } else {
      $('.gs-analysis-spinner').hide();
      GlobalSensitivityAnalyser.analysisCompleted.emit({ node: node, overlayHtml: `<div class="code-error">${result.error.message}</div>` });
    }
